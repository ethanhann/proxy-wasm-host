...
pub(super) fn proxy_done(ctx: &mut impl AsContextMut<Data = HostState>) -> Result<(), Failure> {
    if ctx.as_context_mut().data_mut().contexts_mut().done() {
        Ok(())
    } else {
...
pub(super) fn proxy_set_effective_context(
    ...
    let contexts = ctx.data_mut().contexts_mut();
...
mod tests {
    ...
    fn the_effective_context_changes_only_to_a_known_identifier() {
        ...
        let contexts = instance.state_mut().contexts_mut();
        ...
        assert_eq!(instance.state().contexts().effective(), Some(stream));
    }
    ...
    fn a_refused_root_cannot_become_effective() {
        ...
        assert_eq!(instance.state().contexts().effective(), Some(other));
    }
    ...
    fn done_needs_a_pending_effective_context() {
        ...
        assert_eq!(
            instance.state().contexts().state(root),
