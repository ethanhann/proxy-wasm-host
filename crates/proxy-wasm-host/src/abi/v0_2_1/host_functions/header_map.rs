...
pub(super) fn proxy_set_header_map_pairs(
    ...
    let pairs = guest_pairs(state, bytes)?;
...
mod tests {
    ...
    #[test]
    fn a_header_map_above_the_byte_limit_is_a_bad_argument() {
        // Arrange
        let engine = engine();
        let limits = Limits::new().with_max_decoded_map_bytes(16);
        let (mut instance, _) = hosted_with_limits(&engine, GUEST, two_pairs(), &limits);
        let encoded = big_map(4);
        let (data, size) = write(&mut instance, KEY, &encoded);
        ...
        assert_eq!(result.unwrap(), Status::BadArgument);
        assert_eq!(
            RecordingStream::pairs_in(instance.state_mut()).len(),
            2,
            "the map must not change"
        );
    }
